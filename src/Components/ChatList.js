...
import { saveChatList, saveChatId, savePrevChats, saveNewChats, saveConversationList } from '../Redux/Reducers/appSlice';
    ...
    return (
        <>
                ...
                <div className="inner-bk-box">
                    {chatList && chatList.length > 0 &&
                        chatList.map((item, index) =>
                            < a key={index + 1} data-testid="convo-item"
                                ...
                                <div className="relative max-h-5 flex-1 overflow-hidden text-ellipsis break-all">{item.chats[0].question}</div>
                                <div
                                    className="absolute inset-y-0 right-0 z-10 w-8 rounded-r-md bg-gradient-to-l from-gray-900 group-hover:from-gray-700/70">
                                </div>
                            </a >
                        )
                    }
            ...
            <div className="group-open-v" data-headlessui-state="open">
                <button onClick={() => setActiveDropdownIndex(!activeDropdownIndex)} className="group-ui-open dropdown--up" type="button" id="profile_btn">
                    ...
                    <div className="profile-name">
                        {user.name}</div><svg stroke="currentColor" fill="none" strokeWidth="2"
                ...
                <div className={`log-out-area dropdown--item ${activeDropdownIndex ? 'active' : ''}`} id="profile_option" role="menu">
                    <div id="headlessui-menu-item" role="menuitem" tabIndex="-1" data-headlessui-state=""><a
                        onClick={handleClearAllChat}
                        className="flex w-full cursor-pointer items-center gap-3 rounded-none px-3 py-3 text-sm text-white transition-colors duration-200 hover:bg-gray-700"><svg
                    ...
                    <div id="headlessui-menu-item" className="log-open" role="menuitem" tabIndex="-1"
                        data-headlessui-state=""><a href="#">
                            <button className=" "
                                onClick={handleLogout}
                                fdprocessedid="otft8"><svg stroke="currentColor" fill="none" strokeWidth="2"
                                    viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"
